...
1. **Test data must always be anonymous.** Never use real names, emails, or any personally identifiable information in tests. Use generic placeholders like `user@example.com`, `John Doe`, etc.
2. ALL code changes follow TDD - Feature requests mid-stream are NOT exceptions. Write test first, then code.
...
13. Commit. Every green + refactored state is a commit. Each commit compiles and passes all tests.
